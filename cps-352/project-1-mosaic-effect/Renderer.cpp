...
void Renderer::blurRegion(Mat& img, const Corners& corners) const
{
	...
	for (int x = insetCorners.p1.x + 1; x <= insetCorners.p2.x + 1; ++x)
	{
		// If this x-value doesn't mark the end of a new square, then continue on from left to right.
		// Also check for whether it's time to fill in horizontal gaps yet.
		if (((x - insetCorners.p1.x) % m_blurDegree != 0) &&
			(x != insetCorners.p2.x + 1))
			continue;

		// But, if it does, then go from top to bottom and blur the whole column of squares one at a time.
		// Again, start with p1 + 1 because we shouldn't ever have squares with area 0.
		for (int y = insetCorners.p1.y + 1; y <= insetCorners.p2.y + 1; ++y)
		{
			// Wait until we find the vertical end of new square...unless this is for filling a gap at the bottom...
			if (((y - insetCorners.p1.y) % m_blurDegree != 0) &&
				(y != insetCorners.p2.y + 1))
				continue;
